...
app.get('/news', async (req, res) => {
  ...
  const url  = `https://newsapi.org/v2/everything?q=tesla&from=2024-01-14&sortBy=publishedAt&apiKey=d251fad8195047e7909e6893e9697e26`;
...
app.get('/cars', async (req, res) => {
  const search = req.query.search || 'camry';
  console.log("search",search);
  const cars = await carsService.getCars(search);
  console.log(cars);
  res.render('cars', { cars });
});

app.get('/aircraft', async (req, res) => {
  const manufacturer = req.query.manufacturer || 'Gulfstream';
  const model = req.query.model || 'G550';
  const aircrafts = await aircraftService.getAirCrafts(manufacturer, model);
  res.render('aircraft', { aircrafts });
});
